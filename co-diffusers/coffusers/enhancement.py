...
class LoraEnhancerMixin:
    ...
    def set_lora(self,lora_uri,lora_name,weight=0.4):
        set_lora(self.__oins__,lora_uri,lora_name,weight)
        self.lora_dict.update({lora_name:weight})

    def set_lora_weight(self,lora_name,weight):
        self.__oins__.set_adapters([lora_name],adapter_names=[weight])
        self.lora_dict.update({lora_name:weight})
...
class PipelineEnhancer(LoraEnhancerMixin,CLIPEnhancerMixin,EasyInitSubclass):
    overrides = ["to"]
    ...
    def __call__(self,**kwargs):
        ...
        for lora,weight in self.lora_dict.items():
            if lora not in prompt_str:
                ...
                print(f"{lora}:{weight} is disable due to {lora} is not in prompts.")
