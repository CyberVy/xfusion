...
class PipelineEnhancer(CLIPEnhancerMixin):
    ...
    def __init__(self,pipeline):
        ...
        self.overrides = ["__call__"]

    def __call__(self,**kwargs):
        kwargs.update(self.get_embeds_from_pipe(kwargs.get("promot"),kwargs.get("negative_prompt")))
        return self.pipeline.__call__(**kwargs)

    def _(self, item):
        if item not in object.__getattribute__(self,"overrides"):
            object.__getattribute__(self, "pipeline").__getattribute__(item)
        else:
