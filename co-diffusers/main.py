...
set_lora(pipe,lora,lora_name="hands",scale=0.4,download_kwargs={"headers":{"cookie":cookie},"directory":"lora"})

# 2
def generate(prompt,negative_prompt,width,height,num_inference_steps,guidance_scale,clip_skip,num,seed=None,**kwargs):
    seeds = []
    if seed:
        seeds.append(seed)
    else:
    ...
    for item in seeds:
        ...
        image = pipe(prompt,negative_prompt=negative_prompt,width=width,height=height,num_inference_steps=num_inference_steps,guidance_scale=guidance_scale,clip_skip=clip_skip,num_images_per_prompt=1,**kwargs).images[0]
        torch.cuda.empty_cache()
        threading.Thread(target=lambda: send_PIL_photo(image,file_name="Colab.PNG",file_type="PNG",caption=f"Prompt:{prompt}\n\nNegative Prompt:{negative_prompt}\n\nStep:{num_inference_steps},CFG:{guidance_scale},CLIP Skip:{clip_skip}\nSampler:{pipe.scheduler.config._class_name}\nSeed:{item}")).start()
...
pipe.set_adapters(["hands"], [0.4])

generate(prompt,negative_prompt,width,height,num_inference_steps,guidance_scale,clip_skip,num,seed)
