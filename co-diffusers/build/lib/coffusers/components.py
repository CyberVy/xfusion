...
def set_lora(pipe,lora_uri,lora_name,scale=0.5,download_kwargs=None):
...
def get_clip_from_pipe(pipe):
    ...
    for i, item in enumerate(tokenizer_names):
        if hasattr(pipe, item):
            tokenizers.append(getattr(pipe, item))
            text_encoders.append(getattr(pipe, text_encoder_names[i]))
