...
class _PipelineEnhancer(CLIPEnhancerMixin):
    overrides = ["to"]
    ...
    def __setattr__(self, key, value):
        if key in self.overrides:
            ...
            return self.pipeline.__setattr__(key,value)
...
class PipelineEnhancer(EasyInitSubclass,CLIPEnhancerMixin):
    overrides = ["to"]
    overrides.extend(EasyInitSubclass.overrides)
    overrides.extend(CLIPEnhancerMixin.overrides)

    def to(self, *args, **kwargs):
        return self.__class__(self.__oins__.to(*args, **kwargs))
