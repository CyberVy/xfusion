...
from numpy import array,ndarray,diag
import numpy as np
from sklearn.linear_model import LinearRegression
from fluctua import ax_master
x:ndarray = array([1,2,4,5,6,3,8,10,9,12,7]).reshape(-1,1)
x = diag(x.ravel())
y = array([3,6,3,4,5,2,1,2,4,2,1]).reshape(-1,1)
reg = LinearRegression()
reg.fit(x,y)
ax = ax_master()
ax.scatter(x.diagonal(),y.ravel())

r = []
for i in range(1,13):
    z = diag(array([i] * len(y)))
    r.append(reg.predict(z).mean().ravel()[0])

ax.scatter(range(1,13),r,color="green")